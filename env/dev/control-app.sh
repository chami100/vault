...
APP_PID=`/bin/ps |/usr/bin/grep "${VAULT_CMD}" |/usr/bin/grep -v grep |/usr/bin/awk '{print $1}'`
...
status_function() {
  ...
  [ ${SILENT} -ne 0 ] && /bin/ps -e |/usr/bin/grep "${VAULT_CMD}" |/usr/bin/grep -v grep
