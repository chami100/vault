...
SERVICE_NAME=vault
VAULT_OPTIONS="proxy -namespace=secrets/vf-eiot/iot-connectivity/iotmc/non-prod -config=vault-proxy_ldap.hcl"
...
VAULT_CMD=$PATH_TO_PROJ_ROOT/vault
PID_PATH_NAME=$PATH_TO_PROJ_ROOT/pidfile

OLDCWD=`/bin/pwd`

# Get the currently running processes that match the service signature
APP_PID=`/bin/ps -eo pid,cmd |/bin/grep "${VAULT_CMD}" |/bin/grep -v grep |/bin/awk '{print $1}'`
...
status_function() {
  ...
  [ ${SILENT} -ne 0 ] && /bin/ps -e |/bin/grep "${VAULT_CMD}" |/bin/grep -v grep
...
start_function() {
  ...
  nohup ${VAULT_CMD} ${VAULT_OPTIONS} &>> /dev/null &
  #echo $! > ${PID_PATH_NAME}
